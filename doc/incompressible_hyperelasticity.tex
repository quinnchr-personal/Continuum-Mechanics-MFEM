...
{\sloppy
The inputs \code{apps/input/finite\_elasticity/verification/homogeneous\_deformations/plane\_strain\_neo\_hookean.yaml} (unit
square \code{apps/mesh/square.msh}, plane-strain extension to $\lambda = 2$
as in Section 5.1),
\code{plane\_stress\_neo\_hookean.yaml} (the same sheet in plane stress: uniaxial
tension to $\lambda = 2$, thickness stretch $\lambda^{-1/2}$, no pressure
unknown) and \code{uniaxial\_neo\_hookean.yaml} (unit cube \code{apps/mesh/cube.msh},
...
free) exist for the neo-Hookean model, together with equibiaxial and
pure-shear cubes (\code{equibiaxial\_neo\_hookean.yaml},
\code{pure\_shear\_neo\_hookean.yaml}; Sections 4.2 and 4.3, the faces
$Z = 0, 1$ free) and the uniaxial symmetry model with rollers; the other five
models run in the same inputs by changing the \code{material} line, and the
script's closed forms cover all six.  Their Dirichlet entries write the affine data
