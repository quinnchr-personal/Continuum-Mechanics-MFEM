...
{\sloppy
The inputs \code{apps/input/homogeneous/plane\_strain\_<model>.yaml} (unit
...
free) exist for all six models.  Their
Dirichlet entries carry a \code{gradient} key, so the data is
$\bu = \bm{G}\bX$ with $\bm{G} = \operatorname{diag}(\lambda_a - 1)$.
