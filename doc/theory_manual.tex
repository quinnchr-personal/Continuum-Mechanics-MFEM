...
% ---------------------------------------------------------------------------
% Notation macros (shared with solid_mechanics_forms.tex and
% incompressible_hyperelasticity.tex where they overlap). Referential
% quantities carry the subscript R; the left Cauchy-Green tensor is bold B.
% ---------------------------------------------------------------------------
...
implemented in the root \file{src/} library of the repository: quasi-static
finite-strain solid mechanics in total Lagrangian form on a fixed reference
mesh, discretised with continuous Galerkin finite elements on MFEM.  It
...
slightly compressible and incompressible), the two-dimensional plane-strain and
plane-stress reductions, the displacement and the mixed
displacement--pressure formulations, boundary conditions and load
scheduling, the element kernels and the dual-number tangents, the
quasi-static load stepper, damped Newton, the algebraic multigrid and
saddle-point linear solvers, and the post-processing of quadrature-point
quantities.  The verification and validation of the implementation against
...
\file{src/kernels/} & the total Lagrangian element integrators
  ...
  face integrator \\
\file{src/kernels/materials/} & the hyperelastic material functors, the
  plane-stress adapter, the dual-number tangent, the material variant and
  the YAML factory \\
...
\begin{itemize}[nosep]
  \item Quasi-static balance of linear momentum in total Lagrangian form:
    the mesh never moves, every integral is over the reference
    configuration.  Loads and prescribed displacements are functions of a
    pseudo-time $t \in [0, 1]$; inertia is absent.
  ...
  \item Solvers: load stepping with bisection on failure, damped Newton
    with Armijo backtracking, GMRES or CG with BoomerAMG, and FGMRES with a
    block upper-triangular preconditioner for the $u$--$p$ saddle point.
  \item Post-processing of Cauchy and first Piola--Kirchhoff stress, the
    deformation gradient, its determinant, von Mises stress, energy density
...
Not implemented: dynamics, internal variables and inelasticity,
...
Two notes in \file{doc/} predate this manual and are its sources where they
overlap: \file{solid\_mechanics\_forms.tex} derives the strong and weak forms
and the follower-pressure tangent; \file{incompressible\_hyperelasticity.tex}
collects the incompressible models and the closed-form homogeneous
solutions used as references.  The verification manual documents every
input and test.
...
1. Compressible, coupled &
  ...
  \code{neo\_hookean}, \code{st\_venant\_kirchhoff} &
...
stress.  Neither model separates the pressure, so neither can be used in
the mixed formulation or at $\nu = \tfrac12$.
...
boundary conditions.  These relations, the neo-Hookean, Mooney--Rivlin,
Yeoh, Gent, Arruda--Boyce and Ogden specialisations, and the closed-form
uniaxial, equibiaxial, pure-shear and simple-shear states that serve as
reference solutions are collected in
\file{doc/incompressible\_hyperelasticity.tex}; the verification manual
lists the inputs that reproduce them.
...
\begin{enumerate}[nosep]
  ...
  \item stop with failure when the line search cannot reduce the residual
    or after \code{max\_it} iterations.
\end{enumerate}
...
truth, from the state $(\bF, \bP, \Psi)$ at each point:
\begin{center}
\small
\begin{tabular}{@{}l l l@{}}
\toprule
...
$\rho_R$ & reference density & \code{material.rho0} \\
...
\file{kernels/materials/*.hpp} & material functors, isochoric helpers, volumetric laws, spectral solver, plane-stress adapter, dual tangent \\
...
\file{solvers/quasi\_static.\{hpp,cpp\}} & load stepping with bisection \\
\file{solvers/newton.\{hpp,cpp\}} & damped Newton with Armijo backtracking \\
\file{solvers/linear\_solver.\{hpp,cpp\}} & GMRES/CG + BoomerAMG with the systems fallback \\
\file{solvers/saddle\_point\_solver.\{hpp,cpp\}} & augmented-Lagrangian FGMRES for the $u$--$p$ Jacobian \\
\bottomrule
\end{tabular}
\end{center}
