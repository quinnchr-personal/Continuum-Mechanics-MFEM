...
model were added under `apps/input/finite_elasticity/verification/homogeneous_deformations/` and the comparison script derives the
...
- Frozen regressions that must not move: `tests/test_benchmarks.cpp` (Cook corner values),
  `apps/homogeneous_compare.py` over `apps/input/finite_elasticity/verification/homogeneous_deformations/*.yaml`, all `tests/test_*`.
...
- Tests (`tests/test_loading.cpp`):
  - Uniaxial cube by symmetry: eighth-symmetry model of `uniaxial_neo_hookean.yaml`
    ...
    run through `homogeneous_compare.py`; add the input under `apps/input/finite_elasticity/verification/homogeneous_deformations/`
