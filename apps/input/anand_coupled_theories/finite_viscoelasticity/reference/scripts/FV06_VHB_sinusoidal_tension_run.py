...
stretch = 1.5
uMax = (stretch-1)*length                     # shear amplitude
# Time history
t      = 0.0                  # start time (s)
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV06_VHB_sinusoidal_tension.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
plt.tight_layout()
plt.savefig("results/VHB_sinusoidal_tension.png", dpi=600)
...
#     plt.plot(timeHist1[0:i]/length + 1, timeHist2[0:i]/(length*length),  c=colors[3], linewidth=2.0)
#     #-------------------------------------------------------------
#     plt.grid(linestyle="--", linewidth=0.5, color='b')
#     ax.set_xlabel(r'$\lambda$',size=14)
#     ax.set_ylabel(r'$T_{R\,22}$, kPa ',size=14)
