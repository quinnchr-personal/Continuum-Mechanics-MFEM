...
# %%
    ...
    Ttot = 2*(stretch-1)/rate
    # time step size
    dt = 0.01*Ttot
    # Create a constant for the time step
    dk = Constant(domain, PETSc.ScalarType(dt))

    # Function for triangle-wave displacement on boundary.
    def dispRamp(t):
        disp = uMax*2*np.abs( (t/Ttot) - np.floor((t/Ttot) + 0.5) ) 
        return disp
    ...
    if MPI.COMM_WORLD.rank == 0:
        ...
        _np.savetxt("hist/FV02_VHB_uniaxial_tension_neq_fixed_rate%g.csv" % rate, _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
