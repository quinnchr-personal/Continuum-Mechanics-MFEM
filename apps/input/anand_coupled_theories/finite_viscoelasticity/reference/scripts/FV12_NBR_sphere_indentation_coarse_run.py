...
ds = ufl.Measure('ds', domain=domain, subdomain_data=facet_tags, metadata={'quadrature_degree':4}) # Note that surface quadrature degree 
                                                                                                   # is higher here, to help resolve the contact.
...
# contact_traction = 2 * k_pen * ppos(r_ind**2 - (x_sp - x_ind)**2 ) * (x_sp - x_ind)
#
...
Res_1  =  inner( Piola, grad(u_test))*dx \
          - dot(contact_traction, u_test)*ds
            # + derivative(contact_energy, u, u_test) * ds
...
# %% ----
# results file name
results_name = "3D_cube_sphere_indent_coarse"
...
zTop_all_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(6))
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist4[ii] = domain.comm.allreduce(float(np.sum(_Cv.array[zTop_all_u3_dofs])), op=MPI.SUM)
    #
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV12_NBR_sphere_indentation_coarse.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
ax=fig.gca()  
plt.plot(timeHist0[0:ind], timeHist1[0:ind], color=colors[3], linewidth=2.0,marker='.', label='Applied')
plt.plot(timeHist0[0:ind],  -timeHist2[0:ind], color=colors[0], linewidth=2.0, marker='.', label ='Measured')
#-------------------------------------------------------------
plt.xlim(-10,500)
# plt.ylim(-20,1600)
#plt.axis('tight')
...
ax.set_ylabel(r'Indenter Depth, mm')
...
plt.tight_layout()
plt.savefig("results/3D_sphere_indent_cantact_verification.png", dpi=600)
#------------------------------
...
# plt.ylim(-30,2)
#plt.axis('tight')
plt.plot(timeHist0[0:ind], -timeHist3[0:ind]/1e3, color=colors[3], linewidth=2.0, marker='.')
plt.grid(linestyle="--", linewidth=0.5, color='b')
ax.set_xlabel(r'Time, s')              
...
fig.set_size_inches(7,5)
plt.tight_layout()
plt.savefig("results/3D_sphere_indentation_force.png", dpi=600)
