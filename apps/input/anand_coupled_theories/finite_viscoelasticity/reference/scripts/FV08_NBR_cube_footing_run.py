...
numSteps = 24 #6*24     # number of steps for application of pressure ( larger number of steps can be used for smoother movies )
...
def pressRamp(t):
    ...
    if t < tRamp:
        press = press_max*2*np.abs( (t/tRamp) - np.floor((t/tRamp) + 0.5) ) 
    else:
        press = 0.0
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV08_NBR_cube_footing.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
