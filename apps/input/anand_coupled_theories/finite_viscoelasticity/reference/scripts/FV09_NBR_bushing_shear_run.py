...
#
with XDMFFile(MPI.COMM_WORLD,"meshes/bushing.xdmf",'r') as infile:
    domain = infile.read_mesh(name="Grid",xpath="/Xdmf/Domain")
    cell_tags = infile.read_meshtags(domain,name="Grid")
domain.topology.create_connectivity(domain.topology.dim, domain.topology.dim-1)

with XDMFFile(MPI.COMM_WORLD, "meshes/facet_bushing.xdmf", "r") as xdmf:
    facet_tags = xdmf.read_meshtags(domain, name="Grid")
    
# Un-comment these lines to use the finer version of the mesh instead, and recover the exact analysis from the book.
...
top = pyvista.UnstructuredGrid(*vtk_mesh(domain, domain.topology.dim-1,facet_tags.indices[facet_tags.values==7]) )
bottom = pyvista.UnstructuredGrid(*vtk_mesh(domain, domain.topology.dim-1,facet_tags.indices[facet_tags.values==8]) )

actor = plotter.add_mesh(top, show_edges=True,color="blue") # top face is blue
actor2 = plotter.add_mesh(bottom, show_edges=True,color="red") # bottom face is green
...
plotter.screenshot("results/bushing_mesh.png")

Image = lambda *a, **k: None
Image(filename='results/bushing_mesh.png') 
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV09_NBR_bushing_shear.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
fig = plt.figure() 
ax=fig.gca()  
plt.plot(timeHist1[0:ind]/L, timeHist2[0:ind]/1e3, c=colors[3], linewidth=2.0, marker='.')

#plt.axis('tight')
plt.grid(linestyle="--", linewidth=0.5, color='b')
ax.set_xlabel(r'$\gamma$',size=14)
ax.set_ylabel(r'Shear Force, N',size=14)
#ax.set_ylim([0, 60])
...
plt.tight_layout()
plt.savefig("results/NBR_bushing_40mm_min.png", dpi=600)
...
#     plt.plot(timeHist1[0:i]/L, timeHist2[0:i]/1e3, c=colors[3], linewidth=2.0)
...
# f = r"media/viscoelastic_bushing_curve.mp4"
