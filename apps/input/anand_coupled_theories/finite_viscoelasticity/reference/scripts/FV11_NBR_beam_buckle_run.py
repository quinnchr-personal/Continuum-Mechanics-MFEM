...
plotter.screenshot("results/buckle_mesh.png")

Image = lambda *a, **k: None
Image(filename='results/buckle_mesh.png') 

# %% ----
# Define the boundary integration measure "ds" using the facet tags,
# also specify the degree of surface quadrature.
ds = ufl.Measure('ds', domain=domain, subdomain_data=facet_tags, metadata={'quadrature_degree':2})

# Define the volume integration measure "dx" 
# also specify the degree of volume quadrature.
...
tau_1   = Constant(domain, 1.0e-3)    #  relaxation time, s
#
...
t        = 0.0       # start time
Ttot     = 0.04       # total simulation time 
...
disp_tot = -0.75    # final tip displacement (mm)
Tcutoff = Ttot/2   # time to stop ramp and hold displacement (s)

# Boundary condition to ramp up displacement

# A "smooth step" function with:
#  value equal to 0 at x=0
#  value equal to 1 at x=1
#  first derivative equal to 0 at x=0 and x=1 (no sudden velocity changes)
#
def smoothstep(x):
    return  3*(x**(2.0)) - 2*(x**(3.0))

# The displacement function, using the smooth step amplitude to avoid shocks:
def dispRamp(t):
    if t<Tcutoff:
        disp = disp_tot*smoothstep(t/Tcutoff)
    else:
        disp = disp_tot
    return disp
...
# Define  tensor-valued functions for Cvalpha.
...
# interpolate the identity onto the tensor-valued Cv functions.
...
# and the (quadrature representation) of the viscous Cvalpha
#
...
pointForEval = np.array([width/2, width/2,length])
...
zBot_u1_dofs = fem.locate_dofs_topological(ME.sub(0).sub(0), facet_tags.dim, facet_tags.find(1))
zBot_u2_dofs = fem.locate_dofs_topological(ME.sub(0).sub(1), facet_tags.dim, facet_tags.find(1))
zBot_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(1))
#
zTop_u1_dofs = fem.locate_dofs_topological(ME.sub(0).sub(0), facet_tags.dim, facet_tags.find(2))
zTop_u2_dofs = fem.locate_dofs_topological(ME.sub(0).sub(1), facet_tags.dim, facet_tags.find(2))
zTop_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(2))

# building Dirichlet BCs
bcs_1 = dirichletbc(0.0, zBot_u1_dofs, ME.sub(0).sub(0))  # u1 fix - zBot
bcs_2 = dirichletbc(0.0, zBot_u2_dofs, ME.sub(0).sub(1))  # u2 fix - zBot
bcs_3 = dirichletbc(0.0, zBot_u3_dofs, ME.sub(0).sub(2))  # u3 fix - zBot
#
bcs_4 = dirichletbc(0.0, zTop_u1_dofs, ME.sub(0).sub(0))       # u1 fix  - zTop
bcs_5 = dirichletbc(0.0, zTop_u2_dofs, ME.sub(0).sub(1))       # u2 fix  - zTop
bcs_6 = dirichletbc(dispCons, zTop_u3_dofs, ME.sub(0).sub(2))  # u3 move - zTop
...
while (round(t + dt, 9) <= Ttot):
    ...
    dispCons.value = dispRamp(t - float(alpha*dt))
    ...
    _Rv = _av(fem.form(Res)); _Rv.assemble()
    timeHist3[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[zTop_u3_dofs])), op=MPI.SUM)
    #
    timeHist1[ii] = dispRamp(t) #  # time history of applied displacement
    #
    timeHist2[ii] = domain.comm.gather(fem.assemble_scalar(rxnForce))[0]# time history of reaction force
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV11_NBR_beam_buckle.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
ax1.plot(timeHist0[0:ind], -timeHist1[0:ind], c=colors[0], linewidth=1.0, marker='.')
...
plt.tight_layout()
plt.savefig("results/Finite_viscoelasticity_beam_buckle.png", dpi=600)
...
#     ax2.plot(timeHist0[0:i], -timeHist2[0:i], c=colors[3], linewidth=1.0, marker='.')
#     ax2.grid(linestyle="--", linewidth=0.5, color='b')
#     ax2.set_ylabel('Force, mN')
#     ax2.set_xlabel('Time (s)')
#     from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
...
#     ax1.plot(timeHist0[0:i], -timeHist1[0:i], c=colors[0], linewidth=1.0, marker='.')
#     ax1.grid(linestyle="--", linewidth=0.5, color='b')
#     ax1.set_ylabel('Displacement, mm')
#     from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
#     ax1.xaxis.set_minor_locator(AutoMinorLocator())
