...
def dispRamp(t):
    disp = uMax*t/Ttot #uMax*2*np.abs( (t/Ttot) - np.floor((t/Ttot) + 0.5) ) 
    return disp
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV01_VHB_uniaxial_tension_eq.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
plt.plot(timeHist1[0:ind]/length + 1, timeHist2[0:ind]/(length*length), c='r', linewidth=2.0, label="Simulation")
#-------------------------------------------------------------
ax.set_xlim(1,10)
# ax.set_ylim(0, 400)
#plt.axis('tight')
...
plt.tight_layout()
plt.savefig("results/finite_viscoelastic_uniaxial_tension_eq.png", dpi=600)


# %% ----
stretchData = np.array([timeHist1[0:ind]/length + 1, timeHist2[0:ind]/(length*length)])
np.savetxt("results/eq_stretch_stress_data.csv", stretchData, delimiter=",")
