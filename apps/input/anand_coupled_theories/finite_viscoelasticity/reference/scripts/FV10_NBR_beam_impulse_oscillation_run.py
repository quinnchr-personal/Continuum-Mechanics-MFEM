...
length = 20. # mm
width  = 2.   # mm
domain = mesh.create_box(MPI.COMM_WORLD, [[0.0,0.0,0.0], [length,width,width]],\
                         [12,6,2], mesh.CellType.tetrahedron)
...
plotter.screenshot("results/impulse_mesh.png")

Image = lambda *a, **k: None
Image(filename='results/impulse_mesh.png') 
...
xBot_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(1))
...
while (round(t + dt, 9) <= Ttot):
    ...
    tracCons.value = tracAmp(t - float(alpha*dt))
    ...
    #
    timeHist1[ii] = tracAmp(t) #domain.comm.gather(fem.assemble_scalar(rxnForce))[0]  # time history of applied traction
    #
    timeHist2[ii] = w.sub(0).sub(1).eval([length, width/2, width/2],colliding_cells[0])[0] # time history of displacement
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV10_NBR_beam_impulse_oscillation.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
ax1.set_ylabel('Shear traction, kPa')
...
plt.tight_layout()
plt.savefig("results/Finite_viscoelasticity_beam_oscillation.png", dpi=600)
...
#     ax1.plot(timeHist0[0:i], timeHist1[0:i], c=colors[3], linewidth=1.0, marker='.')
#     ax1.grid(linestyle="--", linewidth=0.5, color='b')
#     ax1.set_ylabel('Shear traction, kPa')
#     from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
#     ax1.xaxis.set_minor_locator(AutoMinorLocator())
...
#     ax2.plot(timeHist0[0:i], timeHist2[0:i], c=colors[0], linewidth=1.0, marker='.')
#     ax2.grid(linestyle="--", linewidth=0.5, color='b')
#     ax2.set_xlabel('Time (s)')
#     ax2.set_ylabel('Displacement, mm')
#     from matplotlib.ticker import AutoMinorLocator,FormatStrFormatter
...
# f = r"media/viscoelastic_beam_impulse_curve.mp4"
