...
domain = mesh.create_box(MPI.COMM_WORLD, [[0.0,0.0,0.0], [length,length,length]],\
                         [6,6,4], mesh.CellType.tetrahedron)
...
plotter.add_axes(label_size=(0.4, 0.16))
labels = dict(zlabel='Z', xlabel='X', ylabel='Y')
plotter.add_axes(**labels)
    
plotter.screenshot("results/shear_mesh.png")

Image = lambda *a, **k: None
Image(filename='results/shear_mesh.png') 
...
xBot_u1_dofs = fem.locate_dofs_topological(ME.sub(0).sub(0), facet_tags.dim, facet_tags.find(3))
xBot_u2_dofs = fem.locate_dofs_topological(ME.sub(0).sub(1), facet_tags.dim, facet_tags.find(3))
xBot_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(3))
yTop_u1_dofs = fem.locate_dofs_topological(ME.sub(0).sub(0), facet_tags.dim, facet_tags.find(4))
yTop_u2_dofs = fem.locate_dofs_topological(ME.sub(0).sub(1), facet_tags.dim, facet_tags.find(4))
yTop_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(4))
...
bcs_3 = dirichletbc(0.0, xBot_u3_dofs, ME.sub(0).sub(2))  # u3 fix - xBot
#
bcs_4 = dirichletbc(disp_cons, yTop_u1_dofs, ME.sub(0).sub(0))  # disp ramp - yTop
bcs_5 = dirichletbc(0.0, yTop_u2_dofs, ME.sub(0).sub(1))  # u2 fix - yTop
bcs_6 = dirichletbc(0.0, yTop_u3_dofs, ME.sub(0).sub(2))  # u3 fix - yTop
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist3[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[yTop_u1_dofs])), op=MPI.SUM)
    #
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV07_VHB_sinusoidal_shear.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
fig = plt.figure() 
ax=fig.gca()  
plt.plot(timeHist1[0:ind]/length, timeHist2[0:ind]/(length*length), c=colors[3], linewidth=2.0)
#-------------------------------------------------------------
...
ax.set_ylabel(r'$T_{R \, 12}$, kPa ',size=14)
#ax.set_ylim([0, 60])
...
plt.tight_layout()
plt.savefig("results/VHB_shear_hysteresis.png", dpi=600)
...
#     plt.plot(timeHist1[0:i]/length, timeHist2[0:i]/(length*length), c=colors[3], linewidth=2.0)
#     #-------------------------------------------------------------
