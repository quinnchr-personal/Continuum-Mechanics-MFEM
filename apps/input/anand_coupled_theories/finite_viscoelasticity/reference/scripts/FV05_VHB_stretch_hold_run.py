...
uMax = (stretch-1)*length
# Initialize time
t = 0

# input tabular data for time signal
times = np.array([0,    5,  105, 110, 210,  215,  315, 320, 420,  425,  525, 530, 630,  635,  735, 740, 840])
amps  = np.array([0, 0.25, 0.25, 0.5, 0.5, 0.75, 0.75, 1.0, 1.0, 0.75, 0.75, 0.5, 0.5, 0.25, 0.25,   0,   0])
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist3[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[yTop_u2_dofs])), op=MPI.SUM)
    #
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/FV05_VHB_stretch_hold.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
# Here, we import data which was saved from a separate simulation of the 
# "equilibrium" (quasi-static) stress-stretch behavior. 
#
# If the equilibrium branch material parameters are changed, you will need to 
# generate new equilibrium branch data using code FV01.
eq_data = np.genfromtxt('exp_data/eq_stretch_stress_data.csv', delimiter=',')

fig = plt.figure() 
ax=fig.gca()  
plt.plot(timeHist1[0:ind]/length + 1, timeHist2[0:ind]/(length*length), c=colors[3], linewidth=2.0)
plt.plot(eq_data[0,:],  eq_data[1,:], linewidth=1.0, color='k', linestyle="--", label=r"Equilibrium response")
#-------------------------------------------------------------
# ax.set_xlim(1,2.1)
# ax.set_ylim(0, 400)
#plt.axis('tight')
...
plt.tight_layout()
plt.savefig("results/VHB_stretch_hold_stress.png", dpi=600)
...
plt.tight_layout()
plt.savefig("results/VHB_stretch_hold_time.png", dpi=600)
