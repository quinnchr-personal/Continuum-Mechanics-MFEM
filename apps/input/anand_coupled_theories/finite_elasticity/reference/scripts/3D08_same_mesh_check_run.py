...
# %% ----
# results file name
results_name = "3D08_rxn_quad_3D_column_buckling"
...
while (round(t + dt, 9) <= Ttot):
    ...
    # added: the reaction, the residual (no boundary conditions applied) summed over the top-face u3 dofs
    from dolfinx.fem.petsc import assemble_vector as _av
    _Rv = _av(fem.form(Res)); _Rv.assemble()
    timeHist3[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[Top_dofs_u3])), op=MPI.SUM)
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/3D08_rxn_quad.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
