...
Ro = 11  # mm outer radius
Ri = 10  # mm inner radius

# Pull in the mesh *.xdmf file and read any named domains in the mesh.
with XDMFFile(MPI.COMM_WORLD,"meshes/spherical_shell.xdmf",'r') as infile:
...
with XDMFFile(MPI.COMM_WORLD, "meshes/facet_spherical_shell.xdmf", "r") as xdmf:
...
yBot = pyvista.UnstructuredGrid(*vtk_mesh(domain, domain.topology.dim-1,facet_tags.indices[facet_tags.values==12]) )
zBot = pyvista.UnstructuredGrid(*vtk_mesh(domain, domain.topology.dim-1,facet_tags.indices[facet_tags.values==13]) )
xBot = pyvista.UnstructuredGrid(*vtk_mesh(domain, domain.topology.dim-1,facet_tags.indices[facet_tags.values==14]) )
inner_surf = pyvista.UnstructuredGrid(*vtk_mesh(domain, domain.topology.dim-1,facet_tags.indices[facet_tags.values==15]) )
#
actor = plotter.add_mesh(yBot, show_edges=True,color="blue") # yBot face is blue
actor2 = plotter.add_mesh(zBot, show_edges=True,color="red") # zBot is red
actor3 = plotter.add_mesh(xBot, show_edges=True,color="green") # xBot is green
actor4 = plotter.add_mesh(inner_surf, show_edges=True,color="yellow") # inner surface is yellow
...
# %% ----
# results file name
results_name = "3D_sphere_inflate"
...
xBtm_u1_dofs = fem.locate_dofs_topological(ME.sub(0).sub(0), facet_tags.dim, facet_tags.find(14))
yBtm_u2_dofs = fem.locate_dofs_topological(ME.sub(0).sub(1), facet_tags.dim, facet_tags.find(12))
zBtm_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(13))
...
while (round(t + dt, 9) <= Ttot):
    ...
    #
    timeHist2[ii] = w.sub(0).sub(0).eval([Ri, 0.0, 0.0],colliding_cells[0])[0] # time history of displacement
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/3D06_sphere_inflation.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
