...
# %% ----
# results file name
results_name = "3D09_spherical_inclusion_rxn_3D_spherical_inclusion"
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist2[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[yTop_u2_dofs])), op=MPI.SUM) / float(area.value)
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/3D09_spherical_inclusion_rxn.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
