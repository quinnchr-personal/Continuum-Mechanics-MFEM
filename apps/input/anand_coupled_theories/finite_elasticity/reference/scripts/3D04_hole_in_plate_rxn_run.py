...
# %% ----
# results file name
results_name = "3D04_hole_in_plate_rxn_3D_HIP"
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist2[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[xTop_u1_dofs])), op=MPI.SUM)
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/3D04_hole_in_plate_rxn.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
