...
uMax     =  length * gammaAmp # amplitude of displacement. Remember L is the box size 
#
...
fac_p = (J-1)   # quadratic law, as this code's input
#
...
# %% ----
# results file name
results_name = "3D02_tet8_quad_rxn_3D_simple_shear"
...
yBot_u1_dofs = fem.locate_dofs_topological(ME.sub(0).sub(0), facet_tags.dim, facet_tags.find(3))
yBot_u2_dofs = fem.locate_dofs_topological(ME.sub(0).sub(1), facet_tags.dim, facet_tags.find(3))
yBot_u3_dofs = fem.locate_dofs_topological(ME.sub(0).sub(2), facet_tags.dim, facet_tags.find(3))
#
...
#
bcs_4 = dirichletbc(disp_cons, yTop_u1_dofs, ME.sub(0).sub(0))  # u1 disp ramp - yTop
...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist0[ii] = w.sub(0).sub(0).eval([length, length, length],colliding_cells[0])[0] # time history of displacement
    #
    timeHist1[ii] =  domain.comm.gather(fem.assemble_scalar(shearRxnForce))[0] # time history of engineering stress
    # added: the reaction, the residual (no boundary conditions applied) summed over the top-face u1 dofs
    from dolfinx.fem.petsc import assemble_vector as _av
    _Rv = _av(fem.form(Res)); _Rv.assemble()
    timeHist2[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[yTop_u1_dofs])), op=MPI.SUM)
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/3D02_tet8_quad_rxn.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
...
# plot figure
plt.plot(timeHist0[:ind]/length, timeHist1[:ind], c='b', linewidth=2.0, marker='.')
#-------------------------------------------------------------
