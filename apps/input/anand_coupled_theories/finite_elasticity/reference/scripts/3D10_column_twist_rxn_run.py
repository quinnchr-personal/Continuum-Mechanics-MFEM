...
# %% ----
# results file name
results_name = "3D10_rxn_3D_column_twisting"
...
while (round(t + dt, 9) <= Ttot):
    ...
    # added: the history the notebook does not record: the twist angle, and the reaction of the bottom face
    # (fixed, so the reference positions are the current ones), the residual (no boundary conditions
    # applied) summed over its nodes: the torque about the z axis (the column axis, through the origin
    # of the section) and the axial force; the top face carries the opposite torque and force
    from dolfinx.fem.petsc import assemble_vector as _av
    _Rv = _av(fem.form(Res)); _Rv.assemble(); _r = _Rv.array
    if ii == 1:
        _Xn = V.tabulate_dof_coordinates()
        _top = np.flatnonzero(np.abs(_Xn[:, 2]) < 1e-8)   # the bottom face
        _v2me = np.asarray(V_to_ME).reshape(-1, 3)
        assert _v2me.shape[0] == _Xn.shape[0]
    _F = _r[_v2me[_top]]
    timeHist0[ii] = thetaRamp(t)
    timeHist1[ii] = domain.comm.allreduce(float(np.sum(_Xn[_top, 0] * _F[:, 1] - _Xn[_top, 1] * _F[:, 0])), op=MPI.SUM)
    timeHist2[ii] = domain.comm.allreduce(float(np.sum(_F[:, 2])), op=MPI.SUM)
...
if MPI.COMM_WORLD.rank == 0 and _h:
    ...
    _np.savetxt("hist/3D10_column_twist_rxn.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
