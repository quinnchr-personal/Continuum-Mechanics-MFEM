...
while (round(t + dt, 9) <= Ttot):
    ...
    timeHist4[ii] = domain.comm.allreduce(float(np.sum(_Rv.array[yTop_u2_dofs[yTop_u2_dofs < _Rv.array.size]])), op=MPI.SUM)
    #
...
if MPI.COMM_WORLD.rank == 0:
    ...
    _np.savetxt("hist/TE02_axi_thermoelas_stretch_adiabatic.csv", _np.column_stack([v[:_n] for v in _h.values()]), delimiter=",", header=",".join(_h), comments="")
