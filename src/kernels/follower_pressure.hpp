...
void FaceContribution(const mfem::FiniteElement &el, mfem::FaceElementTransformations &Tr,
                      ...
                      mfem::Vector *elvect, mfem::DenseMatrix *elmat)
{
  ...
  for (int q = 0; q < ir.GetNPoints(); q++)
  {
    ...
    const double w = scale * ip.weight * p.Eval(Tr, ip);
    ...
    if (elmat)
    {
      ...
      for (int b = 0; b < dof; b++)
        for (int k = 0; k < dim; k++)
        {
          // Direction: du_k = phi_b, so dF_kj = DS(b, j).
...
{
public:
  FollowerPressureIntegrator(mfem::Coefficient &p, const double *scale)
    : p_(p), scale_(scale) {}
                          ...
                          mfem::Vector &elvect) override
  {
    if (Tr.GetSpaceDim() == 2)
    {
      follower_detail::FaceContribution<2>(el1, Tr, elfun, p_, *scale_, &elvect, nullptr);
    }
    else
    {
                        ...
                        mfem::DenseMatrix &elmat) override
  {
    if (Tr.GetSpaceDim() == 2)
    {
      follower_detail::FaceContribution<2>(el1, Tr, elfun, p_, *scale_, nullptr, &elmat);
    }
    else
    {
...
{
public:
  BlockFollowerPressureIntegrator(mfem::Coefficient &p, const double *scale)
    : p_(p), scale_(scale) {}
                          ...
                          const mfem::Array<mfem::Vector *> &elvect) override
  {
    if (Tr.GetSpaceDim() == 2)
    {
      follower_detail::FaceContribution<2>(*el1[0], Tr, *elfun[0], p_, *scale_, elvect[0], nullptr);
    }
    else
    {
                        ...
                        const mfem::Array2D<mfem::DenseMatrix *> &elmats) override
  {
    if (Tr.GetSpaceDim() == 2)
    {
      follower_detail::FaceContribution<2>(*el1[0], Tr, *elfun[0], p_, *scale_, nullptr, elmats(0, 0));
    }
    else
    {
