...
class AmaryllisRecessionHistory
{
public:
   void LoadFromMassFile(const string &path)
   {
      times_.clear();
      recession_.clear();
      ...
      if (!in)
      {
         throw runtime_error("Failed to open Amaryllis mass file: " + path);
      }
      ...
      while (std::getline(in, line))
      {
         ...
         // Columns: time, m_dot_g, m_dot_c, front98, front2, recession
         std::istringstream iss(line);
         double t = 0.0;
         double mdotg = 0.0;
         double mdotc = 0.0;
         ...
         double front2 = 0.0;
         double rec = 0.0;
         if (!(iss >> t >> mdotg >> mdotc >> front98 >> front2 >> rec))
         {
            continue;
         }
         ...
         if (t < 0.0)
         {
            throw runtime_error("Negative time in Amaryllis mass file at line " +
                                to_string(line_no) + ": " + path);
         }
         if (!times_.empty() && t < times_.back())
         {
            throw runtime_error(
               "Amaryllis mass file times must be nondecreasing at line " +
               to_string(line_no) + ": " + path);
         }
         if (!std::isfinite(rec))
         {
            throw runtime_error("Non-finite recession value in Amaryllis mass file at line " +
                                to_string(line_no) + ": " + path);
         }
      ...
      if (times_.empty())
      {
         throw runtime_error("Amaryllis mass file is empty or unreadable: " + path);
      }
   }
   ...
   double EvalRecession(const double time) const
   {
      ...
      {
         throw runtime_error(
            "AmaryllisRecessionHistory::EvalRecession() called before LoadFromMassFile().");
      }
      if (time <= times_.front())
      {
...
struct RecessionComparisonMetrics
{
   double rmse = 0.0;
   double max_abs = 0.0;
...
RecessionComparisonMetrics ComputeRecessionComparisonMetrics(
   const vector<double> &driver_recession,
   const vector<double> &amaryllis_recession)
{
   if (driver_recession.size() != amaryllis_recession.size())
   {
      throw runtime_error(
         "Recession comparison vectors must have identical length.");
   }
   if (driver_recession.empty())
   {
      throw runtime_error("Recession comparison vectors are empty.");
   }

   double sum_sq = 0.0;
   double max_err = 0.0;
   for (size_t i = 0; i < driver_recession.size(); ++i)
   {
      const double err = driver_recession[i] - amaryllis_recession[i];
      const double abs_err = std::abs(err);
      sum_sq += err * err;
      max_err = std::max(max_err, abs_err);
   }

   RecessionComparisonMetrics m;
   m.rmse = std::sqrt(sum_sq / static_cast<double>(driver_recession.size()));
   m.max_abs = max_err;
   m.final_abs = std::abs(driver_recession.back() - amaryllis_recession.back());
   m.final_rel =
      m.final_abs / std::max(std::abs(amaryllis_recession.back()), 1.0e-12);
   return m;
}
...
void LoadParams(const string &path, DriverParams &p)
{
   ...
   if (n["strict_case2_2"]) { p.strict_case2_2 = n["strict_case2_2"].as<bool>(); }
                                             ...
                                             const bool enable_blowing)
{
   ...
   for (int it = 0; it < 8; ++it)
   {
      ...
      if (chemistry_on && !strict_case2_2)
      {
   ...
   const SurfaceFluxModelParams &model)
{
   ...
   out.terms.BprimeC = (chemistry_on && !model.strict_case2_2) ? lookup.bc : 0.0;
   out.terms.h_w = h_w;
   ...
   const bool compute_surface_terms)
{
   ...
   for (int be = 0; be < pmesh.GetNBE(); ++be)
   {
      ...
      for (int q = 0; q < ir_face.GetNPoints(); ++q)
      {
         ...
         {
            FT->Face->Transform(fip, face_pos);
         ...
         if (compute_surface_terms)
         {
            ...
            const double m_dot_c =
               (bc_state.chemistryOn != 0) ?
                  ...
                  0.0;
            local_sum.m_dot_c_surf += ds * m_dot_c;
            local_sum.rho_s_surf += ds * solid.rho_s;
   ...
   Vector &top_recession_velocity_true)
{
   ...
   Vector elT, elp, shape_T, shape_p, shape_scalar, gradp, normal;
   DenseMatrix dshape_p;
   const int dim = pmesh.Dimension();
   ...
   for (int be = 0; be < pmesh.GetNBE(); ++be)
   {
      ...
      for (int q = 0; q < ir_face.GetNPoints(); ++q)
      {
         ...
         for (int j = 0; j < dofs_scalar.Size(); ++j)
         {
            ...
            if (dof < 0) { dof = -1 - dof; }
            const double w = ds * shape_scalar[j];
            vel_num(dof) += w * v_rec;
            vel_den(dof) += w;
         }
      }
                          ...
                          const double x, const double y)
{
   ...
   pmesh.FindPoints(pt, elem_ids, ips, false);
...
VectorDivSample SampleVectorFieldAndDivergenceAtPoint(ParMesh &pmesh,
                                                      const ParGridFunction &gf,
                                                      const double x,
                                                      const double y)
{
   const int sdim = pmesh.SpaceDimension();
   DenseMatrix pt(sdim, 1);
   pt = 0.0;
   pt(0, 0) = x;
   if (sdim > 1) { pt(1, 0) = y; }
   ...
   if (elem_ids.Size() > 0 && elem_ids[0] >= 0)
   {
      ElementTransformation *Tr = pmesh.GetElementTransformation(elem_ids[0]);
      MFEM_VERIFY(Tr != nullptr, "Null element transformation in vector point sample.");
      Tr->SetIntPoint(&ips[0]);

      Vector w_val(sdim);
      gf.GetVectorValue(*Tr, ips[0], w_val);
   ...
   int global_count = 0;
   MPI_Allreduce(local_vals, sums, 3, MPI_DOUBLE, MPI_SUM, pmesh.GetComm());
   MPI_Allreduce(&local_found, &global_count, 1, MPI_INT, MPI_SUM, pmesh.GetComm());
...
MassEqProbeSample SampleMassEqProbeAtPoint(ParMesh &pmesh,
                                           const ParFiniteElementSpace &fes_T,
                                           const ParFiniteElementSpace &fes_p,
                                           ...
                                           const double y)
{
   ...
   if (elem_ids.Size() > 0 && elem_ids[0] >= 0)
   {
      const int elem = elem_ids[0];
      const FiniteElement *fe_T = fes_T.GetFE(elem);
      const FiniteElement *fe_p = fes_p.GetFE(elem);
      ElementTransformation *Tr = pmesh.GetElementTransformation(elem);
      MFEM_VERIFY(fe_T && fe_p && Tr, "Null FE/transformation in mass-equation point sample.");

      const IntegrationPoint &ip = ips[0];
      Tr->SetIntPoint(&ip);

      Array<int> dofs_T, dofs_p;
      Vector elT, elp, shape_T, shape_p, gradp;
      DenseMatrix dshape_p;
      ...
      shape_T.SetSize(fe_T->GetDof());
      shape_p.SetSize(fe_p->GetDof());
      dshape_p.SetSize(fe_p->GetDof(), pmesh.Dimension());
      gradp.SetSize(pmesh.Dimension());
      ...
      const double gradp_y = (gradp.Size() > 1) ? gradp[1] : 0.0;
...
int main(int argc, char *argv[])
{
   ...
   try
   {
      ...
      surface_model.strict_case2_2 = params.strict_case2_2;
      ...
      auto write_outputs = [&](const int step, const double time)
      {
         ...
         if (myid == 0)
         {
            ...
            if (use_amaryllis_recession_history)
            {
               ...
               amaryllis_recession_cmp_driver.push_back(recession_total);
      ...
      if (myid == 0)
      {
         ...
         {
            const RecessionComparisonMetrics rec_cmp =
               ComputeRecessionComparisonMetrics(amaryllis_recession_cmp_driver,
                                                amaryllis_recession_cmp_ref);
            const bool rec_rmse_pass =
               (rec_cmp.rmse <= params.tol_recession_rmse);
            const bool rec_final_rel_pass =
            ...
            cout << "Amaryllis recession comparison: "
                 ...
                 << ", final_abs=" << rec_cmp.final_abs
                 << ", final_rel=" << rec_cmp.final_rel
                 << ", pass=" << (rec_pass ? "true" : "false") << endl;
         }
