...
private:
                             ...
                             const TACOTMaterial::GasProperties &gas_old) const
   {
      ...
      const double ale_energy_storage =
         solid.rho_s * solid.cp * T + solid.eps_g * (gas.rho * gas.h - p);
                                   ...
                                   DenseMatrix &J11) const
   {
      ...
      for (int q = 0; q < ir.GetNPoints(); ++q)
      {
         ...
         if (ale_enabled_ && mesh_velocity_coeff_)
         {
...
int main(int argc, char *argv[])
{
   ...
   try
   {
      ...
      Vector top_recession_velocity_true;
      const bool ale_active = (params.ale_enabled && params.moving_mesh);
      if (params.moving_mesh)
      {
      ...
      if (myid == 0)
      {
         ...
         boundary_csv << "time,m_dot_g_surf,m_dot_c_surf,rho_s_surf,"
