// G1 physics gates. The generated Exasim model headers are compiled in as
// the oracle: with identical parameter values the ported kernels must match
// them to round-off, with the q = +grad(u) convention mapped explicitly.
...
// Legacy Exasim mu[] deck rebuilt from the typed parameters at full
// precision. The oracle reads gamma/Re/Pr/M from mu[0..3], the freestream
// state from mu[4..7], nondimensional T_inf from mu[8], and the dimensional
// temperatures from mu[9..10] — feeding it full-precision values makes the
// oracle and the typed code exactly comparable.
std::array<double, 11> LegacyMu(const PerfectGasParams &p)
{
   return {{p.gamma, p.reynolds, p.prandtl, p.mach,
            1.0, 1.0, 0.0, p.TinfND() + 0.5, p.TinfND(),
            p.T_inf_K, p.Twall_K}};
}
...
{
   const double gam = params.gamma;
   const double p = hycfd::Pressure(u, params);
...
// Random admissible state; gradient slots are in the PHYSICAL convention
// q = +grad(u).
...
// including the negative-pressure regime AV continuation transients visit.
...
// uq_model[i>=4] = -uq_phys[i>=4]; states in slots 0..3 are shared.
void ToModelConvention(const double uq_phys[12], double uq_model[12])
{
   for (int i = 0; i < 4; ++i) { uq_model[i] = uq_phys[i]; }
   for (int i = 4; i < 12; ++i) { uq_model[i] = -uq_phys[i]; }
}

void ReferenceFlux(const double uq_model[12], double av, const double *mu,
                   double f[8], double jac[96])
{
   ...
   exasim_reference::PdeModel::flux(
      f, x, uq_model, v, w, mu, uinf, 0.0);
   exasim_reference::PdeModel::flux_jac_uq(
      jac, x, uq_model, v, w, mu, uinf, 0.0);
}

void ReferenceFluxFloored(const double uq_model[12], double av,
                          const double *mu, double f[8], double jac[96])
{
   const double x[2] = {0.0, 0.0};
...
void ReferenceBoundary(int ib, const double uq_model[12],
                       const double uhat[4], const double normal[2],
                       const double *mu, double tau_value,
                       double fb[4], double jac_uq[48], double jac_uh[16])
{
   const double x[2] = {0.0, 0.0};
   const double v[1] = {0.0};
   const double w[1] = {0.0};
   const double tau[1] = {tau_value};
   const double uinf[1] = {0.0};
   exasim_reference::PdeModel::fbou_hdg(
      fb, ib, x, uq_model, v, w, uhat, normal, tau, mu, uinf, 0.0);
   exasim_reference::PdeModel::fbou_hdg_jac_uq(
      jac_uq, ib, x, uq_model, v, w, uhat, normal, tau, mu, uinf, 0.0);
   exasim_reference::PdeModel::fbou_hdg_jac_uh(
      jac_uh, ib, x, uq_model, v, w, uhat, normal, tau, mu, uinf, 0.0);
}

void CheckReferenceParity()
{
   PerfectGasParams params;
   const std::array<double, 11> mu = LegacyMu(params);
   ...
   {
      double uq_phys[12], uhat[4];
      FillAdmissibleState(rng, uq_phys, uhat);
      double uq_model[12];
      ToModelConvention(uq_phys, uq_model);
      ...
      ReferenceFlux(uq_model, av, mu.data(),
                    reference_flux, reference_flux_jac);

      // Model-convention transcription parity.
      double model_flux[8], model_flux_jac[96];
      hycfd::ModelFlux(uq_model, av, params, model_flux, model_flux_jac);
      max_flux_value_error =
         std::max(max_flux_value_error,
                  ScaledError(model_flux, reference_flux, 8));
      max_flux_jac_error =
         std::max(max_flux_jac_error,
                  ScaledError(model_flux_jac, reference_flux_jac, 96));

      // Physical-convention wrapper parity: values identical, gradient
      // Jacobian columns negated.
      double physical_flux[8], physical_jac[96];
      ...
      hycfd::NSFlux(uq_phys, av, params, physical_flux, physical_jac);
      max_flux_value_error =
         std::max(max_flux_value_error,
      ...
      for (int ib = 1; ib <= 3; ++ib)
      {
         ...
         hycfd::NSFbouHdg(ib, uq_phys, uhat, normal, params, actual_fb,
                          actual_uq_jac, actual_uh_jac);
         ReferenceBoundary(ib, uq_model, uhat, normal, mu.data(),
                           params.tau, reference_fb,
                           reference_uq_jac, reference_uh_jac);
         max_fb_value_error =
            std::max(max_fb_value_error,
                     ScaledError(actual_fb, reference_fb, 4));
         // The boundary residuals depend only on u and uhat (gradient
         // columns are zero in both conventions), so the Jacobians are
         // directly comparable.
         max_fb_uq_jac_error =
...
void CheckFiniteDifferences()
{
   ...
   for (int sample = 0; sample < 120; ++sample)
   {
      ...
      hycfd::NSFlux(uq, av, params, base_flux, analytic);
      ...
      for (int variable = 0; variable < 12; ++variable)
      {
         ...
         hycfd::NSFlux(plus_state, av, params, plus_flux);
         hycfd::NSFlux(minus_state, av, params, minus_flux);
         for (int output = 0; output < 8; ++output)
         {
      ...
      for (int ib = 1; ib <= 3; ++ib)
      {
         ...
         hycfd::NSFbouHdg(ib, uq, uhat, normal, params, fb,
         ...
         for (int variable = 0; variable < 12; ++variable)
         {
            ...
            hycfd::NSFbouHdg(ib, plus_state, uhat, normal, params, plus_fb);
            hycfd::NSFbouHdg(ib, minus_state, uhat, normal, params,
                             minus_fb);
         ...
         for (int variable = 0; variable < 4; ++variable)
         {
            ...
            minus_uh[variable] -= h;
            double plus_fb[4], minus_fb[4];
            hycfd::NSFbouHdg(ib, uq, plus_uh, normal, params, plus_fb);
            hycfd::NSFbouHdg(ib, uq, minus_uh, normal, params, minus_fb);
            for (int output = 0; output < 4; ++output)
            {
               const double fd = (plus_fb[output] - minus_fb[output]) /
                                 (2.0 * h);
...
void CheckYReflectionEquivariance()
{
   ...
   for (int sample = 0; sample < 1000; ++sample)
   {
      ...
      const double av = av_dist(rng);
      hycfd::NSFlux(state, av, params, flux);
      hycfd::NSFlux(reflected, av, params, reflected_flux);
...
void CheckDissipativity()
{
   ...
   double maximum_entropy_production =
   ...
   for (int sample = 0; sample < 500; ++sample)
   {
      ...
      // With q = +grad(u) the physical gradient enters the state directly.
      ...
      hycfd::NSFlux(uq, 0.025, params, flux);
      hycfd::NSFlux(zero_gradient_uq, 0.0, params, inviscid_flux);
      ...
      for (int direction = 0; direction < 2; ++direction)
      {
         ...
         Require(hycfd::Pressure(plus_state, params) > 0.0 &&
                 hycfd::Pressure(minus_state, params) > 0.0,
   ...
   Require(maximum_entropy_production <= 1.0e-11,
           "viscous plus AV flux has positive entropy production;"
           " gradient sign convention is wrong");
...
{
   PerfectGasParams params;
   const double pinf = 1.0 / (params.gamma * params.mach * params.mach);
   double freestream[4];
   params.Freestream(freestream);
   const double p = hycfd::Pressure(freestream, params);
   const double viscosity = hycfd::SutherlandMu(p, freestream[0], params);
   const double p_error = std::abs(p - pinf);
   const double mu_error = std::abs(viscosity - 1.0 / params.reynolds);
   Require(p_error <= 8.0 * std::numeric_limits<double>::epsilon(),
           "full-precision freestream pressure identity failed");
   Require(mu_error <=
           8.0 * std::numeric_limits<double>::epsilon() / params.reynolds,
           "full-precision freestream viscosity identity failed");
   // Continuity with the frozen replica: its rounded-deck wall temperature
   // was 0.06550101502128686; the full-precision value must stay within the
   // rounding distance (~1.1e-5 relative), not drift further.
   const double legacy_tiso = 0.06550101502128686;
   const double tiso_shift =
      std::abs(params.TisoW() - legacy_tiso) / legacy_tiso;
   Require(tiso_shift <= 5.0e-5,
           "full-precision TisoW drifted from the frozen replica value");
   std::cout << "PASS freestream identities:"
             << " pressure_abs_error=" << p_error
             << " viscosity_abs_error=" << mu_error
             << " TisoW=" << params.TisoW()
             << " TisoW_shift_vs_frozen=" << tiso_shift << '\n';
}
...
void CheckRegularizedParity()
{
   ...
   {
      double uq_phys[12], uhat[4];
      if (sample < 1000) { FillAdmissibleState(rng, uq_phys, uhat); }
      else { FillFloorStraddlingState(rng, uq_phys); }
      double uq_model[12];
      ToModelConvention(uq_phys, uq_model);
      const double av = av_dist(rng);
      ...
      hycfd::ModelFlux(uq_model, av, params, actual_flux, actual_jac);
      ReferenceFluxFloored(uq_model, av, mu.data(),
                           reference_flux, reference_jac);
   ...
   Require(max_jac_error <= tolerance,
           "regularized flux Jacobian does not match generated"
           " floored model");
...
{
   PerfectGasParams params;
   PerfectGasParams floored = params;
   ...
   for (int sample = 0; sample < 1000; ++sample)
   {
      ...
      hycfd::NSFlux(uq, av, params, plain_flux, plain_jac);
      hycfd::NSFlux(uq, av, floored, reg_flux, reg_jac);
   ...
   // (b) The AV-continuation failure regime: a trial state with slightly
   // negative pressure must be evaluable with the floors and NaN without
   // them (the Sutherland sqrt(T^3) mechanism).
   ...
   hycfd::NSFlux(failure_state, av, floored, reg_flux, reg_jac);
   ...
   Require(regularized_finite,
           "regularized flux is not finite at the negative-pressure state");
   hycfd::NSFlux(failure_state, av, params, plain_flux);
...
int main()
{
   try
   {
      ...
      std::cout << "ALL test_physics G1 GATES PASSED\n";
      return EXIT_SUCCESS;
   }
