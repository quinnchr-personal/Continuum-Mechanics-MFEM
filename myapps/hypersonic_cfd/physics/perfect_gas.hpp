...
// Calorically perfect gas, nondimensionalized by freestream density,
// velocity, and reference length. All derived constants are computed at
// full precision (no rounded deck literals).
struct PerfectGasParams
{
   double gamma = 1.4;
   double reynolds = 183500.0;
   double prandtl = 0.71;
   double mach = 8.03;
   // Dimensional temperatures [K]: freestream (Sutherland reference) and
   // isothermal-wall target.
   double T_inf_K = 124.49;
   double Twall_K = 294.44;
   double tau = 1.0;
   // pdemodel_ns.m density/pressure regularization (smoothed rho/p floors);
   // required by AV continuation transients, inactive at converged states.
   bool regularized = false;

   // Nondimensional freestream temperature in internal-energy units:
   // T_inf = 1/(gamma*(gamma-1)*M^2).
   double TinfND() const
   {
      return 1.0 / (gamma * (gamma - 1.0) * mach * mach);
   }
   // Freestream conservative state (rho, rho*u, rho*v, rho*E).
   void Freestream(double state[4]) const
   {
      state[0] = 1.0;
      state[1] = 1.0;
      state[2] = 0.0;
   ...
   // Isothermal-wall energy target: (T_wall/T_inf) * T_inf(nondim).
   double TisoW() const { return Twall_K / T_inf_K * TinfND(); }
};

inline double Pressure(const double u[4], const PerfectGasParams &params)
{
   const double inv_r = 1.0 / u[0];
   return (params.gamma - 1.0) *
          (u[3] - 0.5 * inv_r * (u[1] * u[1] + u[2] * u[2]));
}

inline double SutherlandMu(double p, double r, const PerfectGasParams &params)
{
   // The Sutherland constant 110.4 K is baked into the generated Jacobian
   // literals below; keep it a literal here so the two stay consistent.
   const double tr = params.gamma * params.mach * params.mach * p / r;
   const double tphys = params.T_inf_K * tr;
   return std::pow(tr, 1.5) * (params.T_inf_K + 110.4) /
          (params.reynolds * (tphys + 110.4));
}
                                       ...
                                       const PerfectGasParams &params)
{
   ...
   a.fc = a.muphys * gam / params.prandtl;
...
inline void FluxJacobianGenerated(const double uq[12], double av,
                                  const PerfectGasParams &params, double f[96])
{
   ...
   const double v0 = av;
   const double mu0 = params.gamma;
   const double mu1 = params.reynolds;
   const double mu2 = params.prandtl;
   const double mu3 = params.mach;
   const double mu9 = params.T_inf_K;
...
inline void FluxRegularizedGenerated(const double uq[12], double av,
                                     const PerfectGasParams &params, double f[8])
{
   const double uq0 = uq[0];
   const double uq1 = uq[1];
   const double uq2 = uq[2];
...
// Exasim-convention flux: uq[4..11] hold q = -grad(u). Kept verbatim so the
// generated Jacobian transcriptions stay byte-comparable with the oracle
// headers; all solver code uses the physical-gradient NSFlux wrapper below.
inline void ModelFlux(const double uq[12], double av,
                      const PerfectGasParams &params,
...
inline void NSFbouHdg(int ib, const double uq[12], const double uhat[4],
                      const double[2], const PerfectGasParams &params, double fb[4],
...
inline void NSHeatFlux(const double uhat[4], const double uq[12],
                       const PerfectGasParams &params, double f[2])
{
   // uq[4..11] hold the physical gradient q = +grad(u); the transport kernel
   // uses the Exasim sign, so flip on entry. The output is the Exasim "Fint"
   // conductive pair fc*(Tx, Ty) in that model convention, matching the
   // frozen wall reference CSVs.
   double work[12];
...
inline void NSVisScalars(const double u[4], const PerfectGasParams &params,
                         double s[4])
{
   const double inv_r = 1.0 / u[0];
   s[0] = u[0];
   s[1] = u[1] * inv_r;
   s[2] = u[2] * inv_r;
   s[3] = Pressure(u, params);
}

// Physical-gradient flux: uq[4..11] hold q = +grad(u). This is the public
// flux for all solver code; it wraps the Exasim-convention kernels with a
// sign map on the gradient slots and the matching Jacobian columns.
inline void NSFlux(const double uq[12], double av,
                   const PerfectGasParams &params, double f[8],
                   double *dfduq = nullptr)
{
   double uq_model[12];
   std::copy(uq, uq + 4, uq_model);
   for (int i = 4; i < 12; ++i) { uq_model[i] = -uq[i]; }
   ModelFlux(uq_model, av, params, f, dfduq);
   if (dfduq)
   {
